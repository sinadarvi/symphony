...
import { isTerminalCodexEvent, normalizeCodexEvent, type CodexRuntimeEvent } from "./events.js";
...
export class CodexAppServerClient implements CodexSession {
  ...
  async *runTurn(input: RunTurnInput): AsyncIterable<CodexRuntimeEvent> {
    ...
    let terminal = false;
    child.stdin.write(JSON.stringify({ input: input.input, title: input.title, cwd: workspacePath }) + "\n");
